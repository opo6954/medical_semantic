...
Train method
1. train_SPHash
train of SPDH Network with label file to npy file

2. train_SPHash_givenNpy
train of SPDH Network with npy file path

3. train_SPHash_loadDisk
train of SPDH Network with load disk scheme(for too large dataset that exceeds RAM)
...
# train of SPDH Network with npy file path
def train_SPHash_givenNpy(num_classes=6, prefix='', modelSavedPath = '../largeData/model', inputTrainImgNpyFile='', inputTrainLabelNpyFile='', inputTestImgNpyFile='', inputTestLabelNpyFile='',  bitSize=16, num_epochs=30, weightSM=1.0, weightApp=1.0, weightFire=1.0):

    tf.reset_default_graph()

    prefix = prefix + '_' + str(bitSize) + '_wSM_' + str(weightSM) + '_wApp_' + str(
    ...
    trainImg_path = inputTrainImgNpyFile
    trainLabel_path = inputTrainLabelNpyFile
    testImg_path = inputTestImgNpyFile
    testLabel_path = inputTestLabelNpyFile

    print('load train data...')
    trainData = dataManager.DataManager(imgNpyPath=trainImg_path, labelNpyPath=trainLabel_path, batchSize=batchSize,
                                        classNumber=num_classes)
    print('load test data...')
    testData = dataManager.DataManager(imgNpyPath=testImg_path, labelNpyPath=testLabel_path, batchSize=batchSize,
                                       classNumber=num_classes)
...
# def train_SPHash_givenNpy(num_classes=6, prefix='', modelSavedPath = '../largeData/model', inputTrainImgNpyFile='', inputTrainLabelNpyFile='', inputTestImgNpyFile='', inputTestLabelNpyFile='',  bitSize=16, num_epochs=30, weightSM=1.0, weightApp=1.0, weightFire=1.0):
def train_SPHash_loadDisk(num_classes=6, prefix='', modelSavedPath = '../kargeData/model', inputTrainLabelFile='', inputTestLabelFile='', bitSize=16, num_epochs=30, weightSM=1.0, weightApp=1.0, weightFire=1.0):
    ...
    trainData = dataManagerFromDisk.DataManagerFromDisk(labelFilePath=inputTrainLabelFile, batchSize=batchSize, totalClass=num_classes)

    print('load test data...')
    testData = dataManagerFromDisk.DataManagerFromDisk(labelFilePath=inputTestLabelFile, batchSize=batchSize, totalClass=num_classes)

    trainDataSize = len(trainData.imgNLabelList)
    testDataSize = len(testData.imgNLabelList)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for step in range(train_batches_per_epoch):
                ...
                input_value, label_value = trainData.returnNextBatch()
            ...
            for _ in range(val_batches_per_epoch):
                img_batch, label_batch = testData.returnNextBatch()
